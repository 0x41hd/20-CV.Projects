...
class LineDetectionApp:
    def __init__(self, root):
        ...
        self.select_button = Button(
            root, text="Choose Image", command=self.select_image)
        self.select_button.pack()

        self.canny_scale = Scale(
            root, from_=50, to=150, orient=HORIZONTAL, label='Canny Threshold')
        self.canny_scale.set(100)
        self.canny_scale.pack()

        self.hough_thresh_scale = Scale(
            root, from_=50, to=200, orient=HORIZONTAL, label='Hough Threshold')
        self.hough_thresh_scale.set(100)
        self.hough_thresh_scale.pack()

        self.detect_button = Button(
            root, text="Detect Lines", command=self.detect_lines)
    ...
    def detect_lines(self):
        ...
        edges = cv2.Canny(gray, self.canny_scale.get(),
                          self.canny_scale.get() * 2)

        lines = cv2.HoughLines(edges, 1, np.pi / 180,
                               self.hough_thresh_scale.get())
